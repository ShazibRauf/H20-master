...
def showObjJoints(imgIn, gtIn, estIn=None, filename=None, upscale=1, lineThickness=3):
    ...
    '''
    import cv2
    jointConns = [[0, 1, 2, 3, 0], [4, 5, 6, 7, 4], [0, 4], [1, 5], [2, 6], [3,7]]
    #jointConns = [[1,0,4,5,1], [2,3,7,6,2], [1,2], [0,3], [5,6], [4,7]]
...
def show3DHandJoints(ax, gtIn):
    ...
    ax.view_init(elev=90, azim=-90)
    for limb_num in range(len(limbs)):
        x1 = gtIn[limbs[limb_num][0], 0]
        y1 = gtIn[limbs[limb_num][0], 1]
        z1 = gtIn[limbs[limb_num][0], 2]
        x2 = gtIn[limbs[limb_num][1], 0]
        y2 = gtIn[limbs[limb_num][1], 1]
        z2 = gtIn[limbs[limb_num][1], 2]
...
def show3DObjCorners(ax, gtIn):
    ...
    for i in range(len(jointConns)):
        for j in range(len(jointConns[i]) - 1):
            jntC = jointConns[i][j]
            jntN = jointConns[i][j+1]
            ax.plot([gtIn[jntC][0], gtIn[jntN][0]], [gtIn[jntC][1], gtIn[jntN][1]], [gtIn[jntC][2], gtIn[jntN][2]], color='k')
...
class Open3DWin():
    def __init__(self):
        ...
        if (self.vis.get_render_option() != None):
...
def open3dVisualize(mList, colorList, hand_faces):
    import open3d
    o3dMeshList = []
    for i, m in enumerate(mList):
        ...
        print(np.asarray(mesh.vertices[:5]), np.asarray(mesh.triangles[:5]))
        if colorList[i] == 'r':
            ...
            mesh.vertex_colors = open3d.utility.Vector3dVector(np.tile(np.array([[0.1, 0.3, 0.3]]), [numVert, 1]))
...
def read_RGB_img(base_dir, seq_name, file_id, split):
    ...
    if os.path.exists(os.path.join(base_dir, seq_name, 'rgb', file_id + '.png')):
        img_filename = os.path.join(base_dir, seq_name, 'rgb', file_id + '.png')
    else:
        img_filename = os.path.join(base_dir, seq_name, 'rgb', file_id + '.jpg')
...
def read_depth_img(base_dir, seq_name, file_id, split):
    ...
    depth_filename = os.path.join(base_dir, seq_name, 'depth', file_id + '.png')
...
def read_annotation(base_dir, seq_name, file_id, split):
    ...
    filename = os.path.join(base_dir, seq_name)
    
    #object corners
    #objCorners3DRest_filename = os.path.join(filename, 'obj_pose', file_id+ '.txt')
    #with open(objCorners3DRest_filename, "r") as file1:
    #    f_list = [float(i) for line in file1 for i in line.split(' ') if i.strip()]
    #    arr = np.array(f_list)

    #    objList = ['background', 'book', 'expresso', 'lotion', 'spray', 'milk', 'cocoa', 'chips', 'cappuccino']
    #    dict['objName'] = objList[int(arr[0])]


    #    arr = arr[4:28]
    #    dict['objCorners3DRest'] = arr.reshape(8,3)



    #camera matrix
    camMatrix_filename = os.path.join(filename, 'obj_pose_rt', file_id+ '.txt')
    with open(camMatrix_filename, "r") as file1:
        f_list = [float(i) for line in file1 for i in line.split(' ') if i.strip()]
        arr = np.array(f_list)
        dict['camMatrix'] = arr[1:].reshape(4,4)
        temp = arr[1:].reshape(4,4)
        dict['objRot'] = temp[:3,:3]


    #hand Pose Mano
    handPose_filename = os.path.join(filename, 'hand_pose_mano', file_id+ '.txt')
    with open(handPose_filename, "r") as file1:
        f_list = [float(i) for line in file1 for i in line.split(' ') if i.strip()]
        arr = np.array(f_list)

        dict['handTrans_right'] = arr[63:66]
        dict['handPose_right1'] = arr[66:114]
        dict['handBeta_right'] = arr[114:124]

        dict['handTrans_left'] = arr[1:4]
        dict['handPose_left1'] = arr[4:52]
        dict['handBeta_left'] = arr[52:62]


    with open(os.path.join(filename,'cam_intrinsics.txt'), "r") as file1:
        f_list = [float(i) for line in file1 for i in line.split(' ') if i.strip()]
        camPose = np.array(f_list)

    camPose = np.array([[camPose[0],0,camPose[2]],[0,camPose[1],camPose[3]],[0,0,1]])
    dict['camMat'] = camPose


    #hand Pose
    handPose_filename = os.path.join(filename, 'hand_pose', file_id+ '.txt')
    with open(handPose_filename, "r") as file1:
        f_list = [float(i) for line in file1 for i in line.split(' ') if i.strip()]
        arr = np.array(f_list)
        #arr = arr[1:64].reshape(21,3)
        #arr = arr[65:128].reshape(21,3)

        print("-------------",arr[1:64].reshape(21,3))
     #   dict['handPose_right'] = arr[65:128].reshape(21,3)


    #Hand Pose Predicted
    handPose_filename = os.path.join('/home5/satti/HOPE-master8-Zeros_TrainAndVal/Visualization/myfile.txt')
    with open(handPose_filename, "r") as file1:
        f_list = [float(i) for line in file1 for i in line.split(' ') if i.strip()]
        arr = np.array(f_list)
        #arr = arr[1:64].reshape(21,3)
        ...
        dict['pred_handPose_left'] = arr[0:63].reshape(21,3)
        dict['pred_handPose_right'] = arr[63:126].reshape(21,3)
        dict['pred_objCorners3DRest'] = arr[126:150].reshape(8,3)


    #Hand Pose
    handPose_filename = os.path.join('/home5/satti/HOPE-master8-Zeros_TrainAndVal/Visualization/myfile_orig.txt')
    with open(handPose_filename, "r") as file1:
        f_list = [float(i) for line in file1 for i in line.split(' ') if i.strip()]
        arr = np.array(f_list)
        #arr = arr[1:64].reshape(21,3)
        ...
        #coordChangeMat = np.array([[1., 0., 0.], [0, -1., 0.], [0., 0., -1.]], dtype=np.float32)
        #print("============",arr)
        #arr = arr.reshape(50,3)
        #arr = arr.dot(coordChangeMat.T) 
        #arr = arr.flatten()

        dict['handPose_left'] = arr[0:63].reshape(21,3)
        dict['handPose_right'] = arr[63:126].reshape(21,3)
        dict['objCorners3DRest'] = arr[126:150].reshape(8,3)
